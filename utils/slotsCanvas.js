 ...
 * Renders at half resolution for performance by default (scaled up via
 * ctx.scale()); noDownscale renders at native resolution for previews.
 */
async function drawSpinAnimation(finalGrid, options = {}) {
  ...
  // Render spin frames at half resolution by default (Pi 3B optimization) and
  // scale the finished frame up for encoding; noDownscale renders natively. In
  // both cases drawing uses full-res coordinates via ctx.scale(scale), so the
  // symbol/lock/motion-blur logic below is shared.
  const scale = noDownscale ? 1 : 0.5;
  const bufW = Math.floor(W * scale);
  const bufH = Math.floor(H * scale);

  const frameCanvas = createCanvas(bufW, bufH);
  const frameCtx = frameCanvas.getContext("2d");
  frameCtx.scale(scale, scale);
  await drawFrame(frameCtx, jackpotDisplay, activeLines, bet, isBonus, isFreePlay, theme, layout);

  const spinCanvas = createCanvas(bufW, bufH);
  const spinCtx = spinCanvas.getContext("2d");

  // At half-res, frames are captured from a full-res canvas the buffer is scaled
  // onto; at native res the spin canvas already is full-res.
  const outCanvas = scale === 1 ? spinCanvas : createCanvas(W, H);
  const outCtx = scale === 1 ? spinCtx : outCanvas.getContext("2d");

  const symSize = Math.min(layout.CELL_W, layout.CELL_H);

  for (let frame = 0; frame < totalFrames; frame++) {
    // Blit the frame background at native buffer size (no scale transform), then
    // draw symbols with the scale transform (full-res coords → buffer pixels).
    spinCtx.drawImage(frameCanvas, 0, 0);
    spinCtx.save();
    spinCtx.scale(scale, scale);

    for (let col = 0; col < 3; col++) {
      const locked = frame >= lockFrames[col];
      ...
      for (let row = 0; row < 3; row++) {
        ...
        if (locked) {
          drawSymbol(spinCtx, finalGrid[row][col], cx, cy, symSize, theme);
        } else {
          const randomSym = Math.floor(Math.random() * symCount);
          spinCtx.save();
          spinCtx.globalAlpha = 0.4;
          drawSymbol(spinCtx, randomSym, cx, cy, symSize, theme);
          spinCtx.globalAlpha = 1.0;
          spinCtx.fillStyle = theme.colors.motionBlurOverlay;
          spinCtx.fillRect(
    ...
    // Scale the buffer up to full resolution for GIF encoding (no-op at native res).
    if (scale !== 1) outCtx.drawImage(spinCanvas, 0, 0, W, H);

    let delay = 80;
    if (lockFrames.includes(frame)) delay = 300;
    else if (frame === totalFrames - 1) delay = 500;

    frames.push({ data: outCtx.getImageData(0, 0, W, H).data, delay });
  }
