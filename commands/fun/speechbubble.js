...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const imageName = image.name ? image.name.slice(0, image.name.lastIndexOf(".")) : `${new Date().getTime()}`;

    const imageBuffer = Buffer.from(await imageResponse.arrayBuffer());

    const xTip = interaction.options.getNumber("x");
    const yTip = interaction.options.getNumber("y");
