    ...
    switch (subcommand) {
      ...
      case "summary":
        ...
        const summaryFields = summarySlice.map((s, i) => {
          ...
          const meta = [s.timestamp && `Generated: <t:${Math.floor(s.timestamp / 1000)}:S>`, s.mergedFrom && `Merged from: ${s.mergedFrom}`].filter(Boolean).join("\n");
      ...
      case "facts":
        ...
        const allFields = Object.entries(factsSource).map(([_, v]) => ({
