...
module.exports = {
  ...
  async execute(interaction) {
    if (!interaction.guildId) {
      return interaction.reply({ embeds: [buildErrorEmbed(interaction.user, interaction.client, "Personas are only available in servers.")], ephemeral: true });
    }
    ...
    if (sub === "create") {
      ...
      if (!NAME_RE.test(name)) {
        return interaction.reply({ embeds: [buildErrorEmbed(interaction.user, interaction.client, "Persona name must be lowercase a-z, 0-9, or hyphens; 1-32 chars; must start with a letter or digit.")], ephemeral: true });
      }
      if (prompt.length === 0 || prompt.length > MAX_PROMPT_LEN) {
        return interaction.reply({ embeds: [buildErrorEmbed(interaction.user, interaction.client, `Prompt must be 1-${MAX_PROMPT_LEN} characters.`)], ephemeral: true });
      }
      if (personas.getByName(guildId, name)) {
        return interaction.reply({ embeds: [buildErrorEmbed(interaction.user, interaction.client, `A persona named **${name}** already exists in this server.`)], ephemeral: true });
      }
      ...
      return interaction.reply({ embeds: [buildSuccessEmbed(interaction.user, interaction.client, `Created persona **${created.name}**.${created.isPublic ? "" : " (private)"} Use \`/persona use ${created.name}\` to pin it.`)], ephemeral: true });
    }

    if (sub === "edit") {
      const name = interaction.options.getString("name").trim().toLowerCase();
      const persona = personas.getByName(guildId, name);
      if (!persona) return interaction.reply({ embeds: [buildErrorEmbed(interaction.user, interaction.client, `No persona named **${name}**.`)], ephemeral: true });
      if (!canModify(interaction, persona)) {
        return interaction.reply({ embeds: [buildErrorEmbed(interaction.user, interaction.client, "You can only edit personas you created.")], ephemeral: true });
      }
      ...
      if (newPrompt === null && newPublic === null) {
        return interaction.reply({ embeds: [buildErrorEmbed(interaction.user, interaction.client, "Pass at least one of `prompt` or `public` to change.")], ephemeral: true });
      }
      ...
      return interaction.reply({ embeds: [buildSuccessEmbed(interaction.user, interaction.client, `Updated persona **${updated.name}**.`)], ephemeral: true });
    }
    ...
    if (sub === "use") {
      ...
      if (!persona.isPublic && persona.creatorId !== interaction.user.id && interaction.user.id !== OWNER_ID) {
        return interaction.reply({ embeds: [buildErrorEmbed(interaction.user, interaction.client, `Persona **${name}** is private.`)], ephemeral: true });
      }
      ...
      return interaction.reply({ embeds: [buildSuccessEmbed(interaction.user, interaction.client, `Pinned persona **${persona.name}** to this channel. The chatbot will speak as them until \`/persona clear\`.`)], ephemeral: true });
    }
    ...
    if (sub === "clear") {
      ...
      if (!ctx?.persona_id) {
        return interaction.reply({ embeds: [buildErrorEmbed(interaction.user, interaction.client, "No persona is pinned to this channel.")], ephemeral: true });
      }
      ...
      return interaction.reply({ embeds: [buildSuccessEmbed(interaction.user, interaction.client, "Persona cleared from this channel.")], ephemeral: true });
    }

    if (sub === "list") {
      const all = personas.listForGuild(guildId);
      if (all.length === 0) {
        return interaction.reply({ embeds: [buildInfoEmbed(interaction.user, interaction.client, "No personas exist in this server yet. Create one with `/persona create`.")], ephemeral: true });
      }
      const ctx = await getThreadContext(interaction.channel);
    ...
    if (sub === "delete") {
      ...
      if (!persona) return interaction.reply({ embeds: [buildErrorEmbed(interaction.user, interaction.client, `No persona named **${name}**.`)], ephemeral: true });
      if (!canModify(interaction, persona)) {
        return interaction.reply({ embeds: [buildErrorEmbed(interaction.user, interaction.client, "You can only delete personas you created.")], ephemeral: true });
      }
      personas.deleteById(persona.id);
      logger.log(`[Persona] ${interaction.user.tag} deleted "${persona.name}"`);
      return interaction.reply({ embeds: [buildSuccessEmbed(interaction.user, interaction.client, `Deleted persona **${persona.name}**. Channels that had it pinned will fall back to defaults on the next chatbot turn.`)], ephemeral: true });
    }
  },
};
